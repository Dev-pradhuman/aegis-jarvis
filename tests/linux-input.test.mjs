...
test('desktop input remains disabled by default and requires exact one-use approval', async () => {
  ...
  try {
    ...
    assert.equal(getTool('computer.type').enabled, process.platform === 'linux' && /KDE|Plasma/i.test(process.env.XDG_CURRENT_DESKTOP || '') && ['wayland', 'x11'].includes(process.env.XDG_SESSION_TYPE || ''));
    ...
    const window = { windowId: 'test-window', appId: 'test-app', processId: 42 };
    const platform = { windows: { supported: true, getActive: async () => window, find: async () => window } };
    const dependencies = { persist, execute, platform };
    const missingKey = await invokeToolRequest(state, { toolId: 'computer.type', input: { text: 'hello' } }, dependencies);
    assert.equal(missingKey.body.code, 'IDEMPOTENCY_KEY_REQUIRED');
    const pending = await invokeToolRequest(state, { toolId: 'computer.type', input: { text: 'hello' }, idempotencyKey: 'input-1' }, dependencies);
    assert.equal(pending.status, 202);
    assert.match(pending.body.approval.sub, /hello/);
    pending.body.approval.status = 'approved';
    const wrong = await invokeToolRequest(state, { toolId: 'computer.type', input: { text: 'other' }, idempotencyKey: 'input-2', approvalId: pending.body.approval.id }, dependencies);
    assert.equal(wrong.status, 403);
    const correct = await invokeToolRequest(state, { toolId: 'computer.type', input: { text: 'hello' }, idempotencyKey: 'input-1', approvalId: pending.body.approval.id }, dependencies);
    assert.equal(correct.status, 200);
    assert.equal(state.approvals[0].status, 'consumed');
    const chatPending = await invokeToolRequest(state, { toolId: 'computer.keypress', input: { keys: 'Enter' }, runId: 'run-chat', idempotencyKey: 'input-chat', resumeOnApproval: true }, dependencies);
    assert.deepEqual(chatPending.body.approval.pendingRequest, { toolId: 'computer.keypress', input: { keys: 'Enter', target: window }, runId: 'run-chat', idempotencyKey: 'input-chat' });
  } finally {
