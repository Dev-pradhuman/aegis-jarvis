...
export async function computerCapabilities(platform = osPlatform) {
  ...
  const [audio, media, clipboard, screenBackend] = await Promise.all([
  ...
  return {
    ...
    keyboard: unavailable(wayland ? 'Wayland remote desktop portal session is not implemented' : 'Desktop input provider is not implemented'),
    mouse: unavailable(wayland ? 'Wayland remote desktop portal session is not implemented' : 'Desktop pointer provider is not implemented'),
